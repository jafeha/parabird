...
complain_dialog_title="Tor Thunderbird Bundle"
...
if [ "`id -u`" -eq 0 ]; then
	complain "The Tor Thunderbird Bundle should not be run as root.  Exiting."
	exit 1
fi
...
exitcode="$?"
if [ "$exitcode" -ne 0 ]; then
	complain "Thunderbird exited abnormally.  Exit code: $exitcode"
	exit "$exitcode"
else
	printf '\nThunderbird exited cleanly.\n'
fi
